...
use na::{Vector3};
use crate::orbital_mechanics::{orbit::{Trajectory, TransferTrajectory}, stumpff::{stumpff_c, stumpff_dcdz, stumpff_dsdz, stumpff_s}};
...
pub fn transfer_trajectory_solver(traj_target: &Trajectory, traj_chaser: &Trajectory, dt: f64) -> Result<(TransferTrajectory, TransferTrajectory), String> {
    assert!(dt >= 0.0, "must propagate orbit forward in time");

    // target at intercept time
    let traj_targ1 = propagate_orbit(&traj_target, dt)?;

    let r1_vec = traj_chaser.r.clone();
    let r2_vec = traj_targ1.r.clone();
    ...
    let v1_short: Vector3<f64> = (r2_vec.clone() - f_short * r1_vec.clone()) / g_short;
    let v2_short: Vector3<f64> = (gdot_short * r2_vec.clone() - r1_vec.clone()) / g_short;
    let traj_short1 = Trajectory {
        r: r1_vec, v: v1_short, mu: mu
    };
    let traj_short2 = Trajectory {
        r: r2_vec, v: v2_short, mu: mu
    };
    ...
    let v1_long: Vector3<f64> = (r2_vec.clone() - f_long * r1_vec.clone()) / g_long;
    let v2_long: Vector3<f64> = (gdot_long * r2_vec.clone() - r1_vec.clone()) / g_long;
    let traj_long1 = Trajectory {
        r: r1_vec, v: v1_long, mu: mu
    };
    let traj_long2 = Trajectory {
        r: r2_vec, v: v2_long, mu: mu
    };

    Ok((
        TransferTrajectory{traj1: traj_short1, traj2: traj_short2},
        TransferTrajectory{traj1: traj_long1, traj2: traj_long2}
    ))
}


pub fn gauss_uv_fg_solver(r1: f64, r2: f64, dt: f64, mu: f64, nu: f64) -> Result<(f64, f64, f64), String> {
    ...
    loop {
        if nr_counter >= nr_max_iter {
            return Err(String::from("Warning: max newton-raphson iterations reached when solving for z"));
        }
        if bs_counter >= bs_max_iter {
            return Err(String::from("Warning: max bisection-search iterations reached when solving for z"));
        }
        ...
        let dtdz = 1.0/mu.sqrt() * ( x.powi(3) * (sprime - 3.0*s*cprime/(2.0*c)) + A/8.0*(3.0*s*y.sqrt()/c + A/x) );
...
mod tests {
    ...
    fn test_transfer_trajectory_solver() {
        ...
        let result = transfer_trajectory_solver(
            &traj_target, 
            &traj0_chaser, 
            dt
        );
